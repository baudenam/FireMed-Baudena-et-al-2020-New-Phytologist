...
!   3- THREE INITIAL CONDITION USED: FOREST (i.c. 2) AND SHRUBLAND/GRASSLAND (i.c. 6) (NB numbers refer to PARAFIRE file) + SUCCESSIONAL MIXTURE (i.c. 8).
!   4- FINAL AVERAGE CALCULATED OVER LAST 20 YEARS

program mainfire_shortterm
use parafire
...
character (len=2)   :: ifracstr
character (len=3)   :: idummystr
...
do id=100,199               ! stochastic fire loop
do iflamm=1,4               ! increasing flammabilities UP TO x3
do initcond=9,9             ! loop on init cond
do ifrac=60,90,15           ! loop on fracR values =ifrac/100 CHANGED R1
do icol=1,4                 ! loop on c1 values
    write(idummystr,'(I3)') id
...
end program mainfire_shortterm
...
!
        ...
        ! NB C is not needed here because they are renormalised to the sum of alphas in the derivs subroutine
